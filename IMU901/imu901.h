...
void imu901_init(void);                                                             //初始化
                     
uint8_t imu901_unpack(uint8_t ch);                                                  //数据解析
void atkpParsing(atkp_t *packet);

void atkpWriteReg(enum regTable reg, uint16_t data, uint8_t datalen);               //写数据
uint8_t atkpReadReg(enum regTable reg, int16_t *data);                              //读数据
