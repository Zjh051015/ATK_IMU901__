...
IMU_Parameter IMU_Data={
    ...
    .Q_bias = 0.003f,
    .R_measure = 0.03f}}};
IMU_Parameter IMU_Data_Without;
...
/**
 * @brief       获取ATK-MS901M所有数据并对X轴和Y轴进行卡尔曼滤波
 * @param       port   : 指定端口
...
void IMU_All_Read(IMU_Parameter *IMU_Data,uint32_t timeout)
{
    ...
    timer = HAL_GetTick();
	atk_ms901m_get_attitude(&IMU_Data->attitude_data,timeout);                           /* 获取姿态角数据 */
	atk_ms901m_get_gyro_accelerometer(&IMU_Data->gyro_data, &IMU_Data->accelerometer_data,timeout); /* 获取陀螺仪、加速度计数据 */
	if ((IMU_Data->attitude_data.pitch < -90 && IMU_Data->Kalman_Angle.KalmanAngleY > 90) || (IMU_Data->attitude_data.pitch > 90 && IMU_Data->Kalman_Angle.KalmanAngleY < -90))
    {
				IMU_Data->Kalman_Angle.Kalman_Y.angle=IMU_Data->attitude_data.yaw;
        IMU_Data->Kalman_Angle.KalmanAngleY=IMU_Data->attitude_data.yaw;
    }
    else
    {
        IMU_Data->Kalman_Angle.KalmanAngleY = Kalman_getAngle(&IMU_Data->Kalman_Angle.Kalman_Y,IMU_Data->attitude_data.roll, IMU_Data->gyro_data.y, dt);
    }
    if (fabs(IMU_Data->Kalman_Angle.KalmanAngleY) > 90)
        IMU_Data->gyro_data.z = -IMU_Data->gyro_data.z;
    IMU_Data->Kalman_Angle.KalmanAngleX = Kalman_getAngle(&IMU_Data->Kalman_Angle.Kalman_X,IMU_Data->attitude_data.pitch , IMU_Data->gyro_data.x, dt);
}
// 计算零漂补偿的函数

void IMU_GET_Data_Without_Referrence(IMU_Parameter *IMU_Data, uint32_t timeout, int times) {
    IMU_Parameter temp_data; // 临时存储每次读取的数据
    static int State=0;
    static IMU_Parameter sum_data = {0}; // 静态变量，用于累加数据
    int i;
    // 初始化sum_data中的所有成员为0
    if(State==0)
    {
        sum_data.attitude_data.pitch = 0;
        sum_data.attitude_data.roll = 0;
        sum_data.attitude_data.yaw = 0;

        sum_data.gyro_data.x = 0;
        sum_data.gyro_data.y = 0;
        sum_data.gyro_data.z = 0;

        sum_data.accelerometer_data.x = 0;
        sum_data.accelerometer_data.y = 0;
        sum_data.accelerometer_data.z = 0;
        State=1;
    // 获取times次数据
    for (i = 0; i < times; i++) {
        IMU_All_Read(&temp_data, timeout); // 假设超时时间为100ms
        // 累加所有数据
        sum_data.attitude_data.pitch += temp_data.attitude_data.pitch/  (float)times;
        sum_data.attitude_data.roll += temp_data.attitude_data.roll/ (float)times;
        sum_data.attitude_data.yaw += temp_data.attitude_data.yaw/(float)times ;

        sum_data.gyro_data.x += temp_data.gyro_data.x/(float)times ;
        sum_data.gyro_data.y += temp_data.gyro_data.y/ (float)times;
        sum_data.gyro_data.z += temp_data.gyro_data.z/ (float)times;

        sum_data.accelerometer_data.x += temp_data.accelerometer_data.x/(float)times ;
        sum_data.accelerometer_data.y += temp_data.accelerometer_data.y/(float)times ;
        sum_data.accelerometer_data.z += temp_data.accelerometer_data.z/(float)times ;
    }
     {
    // 打印角度数据
    printf("Attitude Data - Pitch: %f, Roll: %f, Yaw: %f\n",
           sum_data.attitude_data.pitch,
           sum_data.attitude_data.roll,
           sum_data.attitude_data.yaw);

    // 打印角速度数据
    printf("Gyro Data - X: %f, Y: %f, Z: %f\n",
           sum_data.gyro_data.x,
           sum_data.gyro_data.y,
           sum_data.gyro_data.z);

    // 打印加速度数据
    printf("Accelerometer Data - X: %f, Y: %f, Z: %f\n",
           sum_data.accelerometer_data.x,
           sum_data.accelerometer_data.y,
           sum_data.accelerometer_data.z);
}
	}
    if (times <= 0) {
    times = 1;
    }
    IMU_All_Read(IMU_Data, 100);
    IMU_Data_Without.attitude_data.pitch = IMU_Data->attitude_data.pitch - sum_data.attitude_data.pitch ;
    IMU_Data_Without.attitude_data.roll = IMU_Data->attitude_data.roll - sum_data.attitude_data.roll  ;
    IMU_Data_Without.attitude_data.yaw = IMU_Data->attitude_data.yaw - sum_data.attitude_data.yaw  ;

    IMU_Data_Without.gyro_data.x = IMU_Data->gyro_data.x - sum_data.gyro_data.x ;
    IMU_Data_Without.gyro_data.y = IMU_Data->gyro_data.y - sum_data.gyro_data.y ;
    IMU_Data_Without.gyro_data.z = IMU_Data->gyro_data.z - sum_data.gyro_data.z ;

    IMU_Data_Without.accelerometer_data.x = IMU_Data->accelerometer_data.x - sum_data.accelerometer_data.x ;
    IMU_Data_Without.accelerometer_data.y = IMU_Data->accelerometer_data.y - sum_data.accelerometer_data.y ;
    IMU_Data_Without.accelerometer_data.z = IMU_Data->accelerometer_data.z - sum_data.accelerometer_data.z ;
		if(State==1)
	{
        // 计算IMU_Data的每个成员的绝对值之和
        double total_abs_sum = 0.0;
        total_abs_sum += fabs(IMU_Data_Without.attitude_data.pitch) + fabs(IMU_Data_Without.attitude_data.roll) + fabs(IMU_Data_Without.attitude_data.yaw);
        total_abs_sum += fabs(IMU_Data_Without.gyro_data.x) + fabs(IMU_Data_Without.gyro_data.y) + fabs(IMU_Data_Without.gyro_data.z);
        total_abs_sum += fabs(IMU_Data_Without.accelerometer_data.x) + fabs(IMU_Data_Without.accelerometer_data.y) + fabs(IMU_Data_Without.accelerometer_data.z);
     ...
     if(State==2)
    {
    static float timestamps_A[3]={0,0,0},timestamps_V[3]={0,0,0};
    static float accelerometer_data[3][3]={0},velocity_data[3][3]={0};
    static int v=0,a=0;
    timestamps_A[v]=HAL_GetTick()/1000;
    for(int dimension=0;dimension<3;dimension++)
    {
    accelerometer_data[dimension][v]=*(&IMU_Data_Without.accelerometer_data.x+dimension)*9.8;
    }
    v++;
    if(v==3)
    {
        double h = (timestamps_A[2] - timestamps_A[0]) / 2;
        for(int dimension=0;dimension<3;dimension++)
        {
            *(&IMU_Data_Without.velocity.x+dimension) += (h/3) * (accelerometer_data[dimension][0] + 4*accelerometer_data[dimension][1] + accelerometer_data[dimension][2]);
        }
    for(int dimension=0;dimension<3;dimension++)
        {
            velocity_data[dimension][a]=*(&IMU_Data_Without.velocity.x+dimension);
        }
    v=0;
    timestamps_V[a]=timestamps_A[1];
    a++;
    } 
    if(a==3)
    {
        for(int dimension=0;dimension<3;dimension++)
    {
        ...
        double h = (timestamps_V[2] - timestamps_V[0]) / 2;
        for(int dimension=0;dimension<3;dimension++)
    {
        *(&IMU_Data_Without.displacement.x+dimension) += (h/3) * (velocity_data[dimension][0] + 4*velocity_data[dimension][1] + velocity_data[dimension][2]);
    }
    a=0;
    }
    }
